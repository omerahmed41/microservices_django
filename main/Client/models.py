...
class Client(models.Model):
    ...
    CLIENT_TYPE_CHOICES = [
        (B2C_TYPE, B2C_TYPE),
        (B2B_CORPORATE_TYPE, B2B_CORPORATE_TYPE),
    ]

    user = models.OneToOneField(
        "auth.User", on_delete=models.CASCADE, related_name="client", null=True
    )
    ...
    referral_code = models.CharField(
        "Referral Code", max_length=200, blank=True, null=True, unique=True
    )
    referral_disabled = models.NullBooleanField(default=False)

    contact_preference = models.CharField(
        "contact preference", max_length=200, blank=True, null=True
    )
